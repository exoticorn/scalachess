...
class PolyglotBook(entries: Map[Long, List[PolyglotBook.RawEntry]]) {
  def apply(pos: Position): List[PolyglotBook.Entry] = {
    for {
      p <- entries.get(PolyglotBook.calcPositionHash(pos)).toList
      entry <- p
      move <- Move.parse(pos, entry.move)
    } yield PolyglotBook.Entry(move, entry.weight)
  }

  def randomMove(pos: Position): Option[PolyglotBook.Entry] = {
    val candidates = this(pos)
    val sumWeight = (0 /: candidates)(_ + _.weight)
    def takeWeighted(xs: List[PolyglotBook.Entry], pos: Int): Option[PolyglotBook.Entry] = {
      xs match {
        case x :: rest if x.weight > pos => Some(x)
        case x :: rest => takeWeighted(rest, pos - x.weight)
        case Nil => None
      }
    }
    takeWeighted(candidates, (math.random * sumWeight).floor.toInt)
  }
}

object PolyglotBook {
  case class RawEntry(key: Long, move: String, weight: Int)
  case class Entry(move: Move, weight: Int)

  def apply(filename: String): PolyglotBook = {
    import java.io._
    val in = new DataInputStream(new BufferedInputStream(new FileInputStream(filename)))
    val entries = new collection.mutable.ListBuffer[RawEntry]
    while (in.available() > 0) {
      val key = in.readLong()
      val move = in.readUnsignedShort()
      val weight = in.readUnsignedShort()
      in.skipBytes(4)
      val moveString = "%c%d%c%d%s".format(
        ((move >> 6) & 7) + 'a',
        ((move >> 9) & 7) + 1,
        (move & 7) + 'a',
        ((move >> 3) & 7) + 1,
        promoteMap((move >> 12) & 7))
      entries += RawEntry(key, moveString, weight)
    }
    new PolyglotBook(entries.toList.groupBy(_.key))
  }

  private val promoteMap = Map(0 -> "", 1 -> "N", 2 -> "B", 3 -> "R", 4 -> "Q")

  def calcPositionHash(pos: Position): Long = {
    val pieceHashes = for (sq <- Square.all; Piece(color, piece) <- Some(pos.board(sq))) yield {
      val offset = (colorOffset(color) + pieceOffset(piece)) * 64 + (sq.file - 'a') + (sq.rank - 1) * 8
      random64(offset)
    }
    val castlingHashes = for (sq <- pos.castling; offset <- castlingOffset.get(sq)) yield random64(768 + offset)
    val enPassentHash = pos.ep.filter { sq =>
      val directions = if (pos.turn == 'white) Seq((-1, -1), (1, -1)) else Seq((-1, 1), (1, 1))
      directions.exists(d => (sq +? d).exists(pos.board(_) == Piece(pos.turn, 'pawn)))
    }.map(sq => random64(772 + sq.file - 'a'))
    val turnHash = if (pos.turn == 'white) random64(780) else 0l
    (pieceHashes ++ castlingHashes ++ enPassentHash :+ turnHash).reduce(_ ^ _)
  }

  private val colorOffset = Map('white -> 1, 'black -> 0)
  private val pieceOffset = Map('pawn -> 0, 'knight -> 2, 'bishop -> 4, 'rook -> 6, 'queen -> 8, 'king -> 10)
  private val castlingOffset = Map(Sq('h1) -> 0, Sq('a1) -> 1, Sq('h8) -> 2, Sq('a8) -> 3)

  private val random64 = Array(
